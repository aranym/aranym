...
Serialport::Serialport(const char *device)
{
	...
	oldTBE = 0;
	oldStatus = 0;
	handle = open(device, O_RDWR|O_NOCTTY|O_NDELAY|O_NONBLOCK);/* Raw mode */
	// /dev/ttyS0 by default or /dev/ttyUSB0 : see Serport in ./aranym/config [SERIAL]
	if (handle < 0)
	{
		D(bug("Serialport: Can not open device %s", device));
		return;
	}
}
...
void Serialport::setData(uint8 value)
{
	...
	if (handle >= 0)
	{
		// while((getTBE() & SCC_TBE) == 0); nb = write(handle, &value, 1); if (nb < 0) D(bug("Serialport: setdata problem"));
		// NB: wait for trnsmit buffer empty before single write cancelled (not available with USB)
...
uint16 Serialport::getStatus(int &charcount)
{
	uint16 value;
	uint16 diff;
	int status = 0;
	...
	if (handle >= 0)
	{
		...
		charcount = nbchar;// to optimize input (see UGLY in scc.cpp)
		if (nbchar > 0)
			value = (SCCB_RCV_INT << 8) | SCC_RCA; // RxIC+RBF
		value |= getTBE();  // TxIC
		value |= SCC_TBE;   // fake transmit buffer empty to optimize output (for ttyS0)
		if (ioctl(handle, TIOCMGET, &status) < 0)
		{
			D(bug("Serialport: Can't get status"));
		}
	...
	if (diff & SCC_CTS)
		value |= SCCB_CTS_INT << 8;// ext status IC on CTS change
...
uint16 Serialport::getTBE() // not suited to serial USB
{
	...
	int status = 0;
	if (ioctl(handle, TIOCSERGETLSR, &status) < 0)
	{
		// OK with ttyS0, not OK with ttyUSB0
		//D(bug("Serialport: Can't get LSR"));
		value |= SCC_TBE;	// only for serial USB
	} else if (status & TIOCSER_TEMT)
	{
		value = SCC_TBE;	// this is a real TBE for ttyS0
		if ((oldTBE & SCC_TBE) == 0)
		{
			value |= SCCB_TXR_INT << 8;
