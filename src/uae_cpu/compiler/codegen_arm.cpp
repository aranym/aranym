...
LOWFUNC(WRITE,NONE,2,raw_bsf_l_rr,(W4 d, RR4 s))
{
	...
	TEQ_ri(d, 0);									// teq %[d],#0
	BEQ_i(0);										// beq <zero>
	SUB_rri(d,d,1);									// sub %[d],%[d],#1
	//<zero>:
	TEQ_ri(d,0);									// teq %[d],#0
	BEQ_i(1);										// beq setz
	BIC_rri(REG_WORK2, REG_WORK2, ARM_Z_FLAG);		// bic r3,r3,#0x40000000
	B_i(0);											// b jp
	// <setz>:
	ORR_rri(REG_WORK2, REG_WORK2, ARM_Z_FLAG);		// orr r3,r3,#0x40000000
	// <jp>:
...
LOWFUNC(WRITE,NONE,1,raw_bswap_16,(RW2 r))
{
	...
	BIC_rri(r, r, 0xff00); 						// bic		%[r], %[r], #0xff0000
	BIC_rri(r, r, 0xff); 						// bic		%[r], %[r], #0xff000000
	ORR_rrr(r, r, REG_WORK1);					// orr  	%[r], %[r], r2
#else
...
LOWFUNC(WRITE,NONE,2,raw_bt_l_ri,(RR4 r, IMM i))
{
	...
	ORR_rri(REG_WORK2, REG_WORK2, ARM_C_FLAG); 	// orr        r3, r3, #0x20000000
	B_i(0);                                 	// b          <jp>

	// <clr>:
	BIC_rri(REG_WORK2, REG_WORK2, ARM_C_FLAG); 	// bic        r3, r3, #0x20000000
...
LOWFUNC(NONE,READ,5,raw_cmov_l_rm_indexed,(W4 d, IMM base, RR4 index, IMM factor, IMM cond))
{
	int shft;
	switch(factor) {
	case 1: shft=0; break;
...
LENDFUNC(NONE,READ,5,raw_cmov_l_rm_indexed,(W4 d, IMM base, RR4 index, IMM factor, IMM cond))
...
{
	LDR_rRI(REG_WORK1, RPC_INDEX, 24);    	 	// ldr     r2, [pc, #24]    ; <value>
	LDR_rR(REG_WORK1, REG_WORK1);          		// ldr     r2, [r2]

	LDR_rRI(REG_WORK2, RPC_INDEX, 20);     		// ldr     r3, [pc, #20]    ; <value2>
...
LOWFUNC(WRITE,NONE,2,raw_cmp_l_ri,(RR4 r, IMM i))
{
	LDR_rRI(REG_WORK1, RPC_INDEX, 16);			// ldr	   r2,[pc,#16]

	CMP_rr(r, REG_WORK1);                      	// cmp     r7, r2
...
LENDFUNC(WRITE,NONE,2,raw_cmp_l_ri,(RR4 r, IMM i))
...
/*
	MRS_CPSR(REG_WORK1);						// mrs   r2,cpsr
	CMP_rrASRi(REG_WORK2, d, 32);				// cmp	 r3,r7,asr #32
	BEQ_i(1);									// beq	 clr
	ORR_rri(REG_WORK1, REG_WORK1, ARM_CV_FLAGS);// orr   r2,r2,#0x30000000
	B_i(0);										// b	 jp
	//clr:
	BIC_rri(REG_WORK1, REG_WORK1, ARM_CV_FLAGS);// bic   r2,r2,#0x30000000
	MSR_CPSR_r(REG_WORK1);						// msr   cpsr,r2
*/
}
...
/*	
	MRS_CPSR(REG_WORK1);						// mrs   r2,cpsr
	CMP_rrASRi(REG_WORK2, MUL_NREG1, 32);		// cmp	 r3,r7,asr #32
	BEQ_i(1);									// beq	 clr
	ORR_rri(REG_WORK1, REG_WORK1, ARM_CV_FLAGS);// orr   r2,r2,#0x30000000
	B_i(0);										// b	 jp
	//clr:
...
LOWFUNC(NONE,WRITE,3,raw_mov_l_Ri,(RR4 d, IMM i, IMM offset))
{
	Dif(!isbyte(offset)) abort();

	LDR_rRI(REG_WORK1, RPC_INDEX, 4); 	// ldr	r2, [pc, #4]	; <value>
	if (offset >= 0)
		STR_rRI(REG_WORK1, d, offset);  // str	r2, [r7, #0x54]
	else
		STR_rRi(REG_WORK1, d, -offset); // str	r2, [r7, #-0x54]
	B_i(0);                            	// b	<jp>
...
{
	LDR_rRI(REG_WORK2, RPC_INDEX, 5*4);				// LDR 		r3,[pc,#20] ; <s>
	LDR_rR(REG_WORK1, REG_WORK2);					// LDR 		r2,[r3];
	ORRS_rrr(d, d, REG_WORK1);						// ORRS		r7,r7,r2
...
{
	LDR_rRI(REG_WORK1, RPC_INDEX, 32);        	// ldr	r2, [pc, #32]	; <value>
	LDR_rR(REG_WORK2, REG_WORK1);               // ldr	r3, [r2]

	LDR_rRI(REG_WORK1, RPC_INDEX, 28);        	// ldr	r2, [pc, #28]	; <value2>
...
static inline void raw_jmp_r(RR4 r)
{
	BX_r(r);
}
