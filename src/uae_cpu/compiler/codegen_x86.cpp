...
int EvalException ( LPEXCEPTION_POINTERS blah, int n_except )
{
    ...
    D(panicbug("fault address is 0x%x at 0x%x",addr,i));
    if (!canbang || !currprefs.cachesize) 
    {
	D(panicbug("Not happy! Canbang or cachesize is 0 in SIGSEGV handler!"));
	return EXCEPTION_CONTINUE_SEARCH;
    }

    if (in_handler) 
	D(panicbug("Argh --- Am already in a handler. Shouldn't happen!"));
	...
	if (pr) {
		...
		if ((addr>=0x10000000 && addr<0x40000000) ||
		    (addr>=0x50000000)) {
		    D(panicbug("Suspicious address 0x%x in SEGV handler.",addr));
		}
		if (dir==SIG_READ) {
		    switch(size) {
		...
		D(panicbug("Create jump to %p",veccode));
	    ...
	    D(panicbug("Huh? Could not find trigger!"));
	    return EXCEPTION_CONTINUE_EXECUTION;
	}
    }
    ...
    if (i) {
	for (j=0;j<10;j++) {
	    panicbug("instruction byte %2d is 0x%02x",j,i[j]);
	}
    }
#if 0
...
oh_dear:
	...
	for (j = 0; j < 12; j++) {
	    panicbug("instruction byte %2d is %02x", j, ((uae_u8 *)sc.eip)[j]);
	}
    fflush(stdout);
...
static void raw_init_cpu(void)
{
    ...
    x=cpuid(0);
    maxlev=x.eax;
    D(panicbug("Max CPUID level=%d Processor is %c%c%c%c%c%c%c%c%c%c%c%c",
	      maxlev,
	      x.ebx,
	      x.ebx>>8,
	      x.ebx>>16,
