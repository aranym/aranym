...
static void UpdateDiskParameters(int disk, bool updateCHS)
{
	...
	// output
	snprintf(disk == 0 ? ide0_size : ide1_size, sizeof(ide0_size), "%6d", sizeMB);
	snprintf(disk == 0 ? ide0_cyl : ide1_cyl, sizeof(ide0_cyl), "%5d", cyl);
	snprintf(disk == 0 ? ide0_head : ide1_head, sizeof(ide0_head), "%2d", head);
	snprintf(disk == 0 ? ide0_spt : ide1_spt, sizeof(ide0_spt), "%3d", spt);
}
...
void DlgDisk::init_create_disk_image(int disk)
{
	...
	char text[250];
	snprintf(text, sizeof(text), "Create disk image '%s' with size %ld MB?", cdi_path, sizeMB);
