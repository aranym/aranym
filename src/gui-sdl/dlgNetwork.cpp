...
static SGOBJ dlg[] =
{
	...
	{ SGTEXT, 0, 0, 2,2, 5,1, "ETH0:" },
	{ SGEDITFIELD, 0, 0, 8,2, MIN(sizeof(eth0_tunnel)-1, 5),1, eth0_tunnel },
	{ SGCHECKBOX, SG_SELECTABLE|SG_RADIO, 0, 21,2, 6,1, "P-t-p" },
	{ SGCHECKBOX, SG_SELECTABLE|SG_RADIO, 0, 30,2, 8,1, "Bridge" },
	...
	{ SGTEXT, 0, 0, 2,12, 7,1, "ETH1:" },
	{ SGEDITFIELD, 0, 0, 8,12, MIN(sizeof(eth1_tunnel)-1, 5),1, eth1_tunnel },
...
void Dialog_NetworkDlg()
{
	...
	// init
	bool eth0_bridge = (strcasecmp(eth0->type, "bridge") == 0);
	dlg[ETH0_PTP].state = !eth0_bridge ? SG_SELECTED : 0;
	dlg[ETH0_BRIDGE].state = eth0_bridge ? SG_SELECTED : 0;
	...
	bool eth1_bridge = (strcasecmp(eth1->type, "bridge") == 0);
	dlg[ETH1_PTP].state = !eth1_bridge ? SG_SELECTED : 0;
	dlg[ETH1_BRIDGE].state = eth1_bridge ? SG_SELECTED : 0;
	...
	if (SDLGui_DoDialog(dlg) == APPLY) {
		bool eth0_bridge = (dlg[ETH0_BRIDGE].state & SG_SELECTED);
		...
		bool eth1_bridge = (dlg[ETH1_BRIDGE].state & SG_SELECTED);
