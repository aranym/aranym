...
static void oi(m68k_disasm_info *info, int size, bool immed)
{
	...
	case FPU_SIZE_SINGLE:
		snprintf(str, sizeof(str), "%.17e", make_single(GETUL(info->memory_vma)));
		ps(info, str);
		info->memory_vma += 4;
		break;
	case FPU_SIZE_EXTENDED:
#ifdef NO_LONGDOUBLE_PRINTF
		snprintf(str, sizeof(str), "%.17e", (double) make_extended(GETUL(info->memory_vma), GETUL(info->memory_vma + 4), GETUL(info->memory_vma + 8)));
#else
		snprintf(str, sizeof(str), "%.17Le", make_extended(GETUL(info->memory_vma), GETUL(info->memory_vma + 4), GETUL(info->memory_vma + 8)));
#endif
		ps(info, str);
		info->memory_vma += 12;
		break;
...
#ifdef NO_LONGDOUBLE_PRINTF
		snprintf(str, sizeof(str), "%.17e", (double) make_packed(GETUL(info->memory_vma), GETUL(info->memory_vma + 4), GETUL(info->memory_vma + 8)));
#else
		snprintf(str, sizeof(str), "%.17Le", make_packed(GETUL(info->memory_vma), GETUL(info->memory_vma + 4), GETUL(info->memory_vma + 8)));
#endif
		ps(info, str);
		info->memory_vma += 12;
		break;
	...
	case FPU_SIZE_DOUBLE:
		snprintf(str, sizeof(str), "%.17e", make_double(GETUL(info->memory_vma), GETUL(info->memory_vma + 4)));
		ps(info, str);
		info->memory_vma += 8;
		break;
	case FPU_SIZE_BYTE:
