	...
	uint32 logOp, uint32 interior_style)
{
	...
	if (dest != 0 || screen_type == 0)
		return VdiDriver::fillArea(vwk, x_, y_, w, h, pattern_addr, fgColor, bgColor, logOp, interior_style);
	...
	memptr dest, int32 dx, int32 dy, int32 w, int32 h, uint32 logOp)
{
	...
	memptr destAddress = ReadInt32(dest + MFDB_ADDRESS);
		...
		default:
			if (planes < 16) {
				...
				for(int32 j = 0; j < h; j++) {
					...
					HostScreen::chunkyToBitplane(dataHost + pixelPosition, planes, bitplanePixels);
	...
	memptr dest, int32 dx, int32 dy, int32 w, int32 h, uint32 logOp)
{
	...
	if (logOp == S_ONLY) {
		...
		/* TODO: optimize ALL_WHITE/ALL_BLACK/D_ONLY */
		for(int32 j = 0; j < h; j++)
			for(int32 i = 0; i < w; i++) {
	...
	uint32 logOp, memptr clip)
{
	...
	if (dest != 0 || screen_type == 0)
		return VdiDriver::drawLine(vwk, x1_, y1_, x2_, y2_, pattern, fgColor, bgColor, logOp, clip);
 ...
 * The trivial cases optimization removed.
 *
 * @author STanda
 **/
	...
	uint16 *areaPattern, uint32 fgColor, uint32 bgColor, uint16 logOp )
{
	...
	/* More variable setup */
	BytesPerPixel = sdl_surf->format->BytesPerPixel;
	pitch = sdl_surf->pitch;
	pixel = ((uint8*)sdl_surf->pixels) + BytesPerPixel * (int32)x + pitch * (int32)y;
	...
	uint32 fgColor, uint32 bgColor, uint16 logOp, int cliprect[] )
{
	...
	BytesPerPixel = sdl_surf->format->BytesPerPixel;
	pitch = sdl_surf->pitch;
	pixel = ((uint8*)sdl_surf->pixels) + BytesPerPixel * (int)x1 + pitch * (int)y;
	ppos = 0;
	...
	case 8:
		...
		{
		case MD_REPLACE:
			for (; pixel<pixellast; pixel += BytesPerPixel, ppos++)
	...
	uint16 pattern, uint32 fgColor, uint32 bgColor, uint16 logOp, int cliprect[] )
{
	...
	BytesPerPixel = sdl_surf->format->BytesPerPixel;
	pitch = sdl_surf->pitch;
	pixel = ((uint8*)sdl_surf->pixels) + BytesPerPixel * (int)x + pitch * (int)y1;
