...
*/
	...
	if (bx_options.cdromd.present) {
		...
		BX_HD_THIS s[1].cdrom.cd =
			new LOWLEVEL_CDROM(bx_options.cdromd.dev);
#endif
		...
		if (bx_options.cdromd.inserted) {
			if (BX_HD_THIS s[1].cdrom.cd->insert_cdrom()) {
				...
				BX_HD_THIS s[1].cdrom.capacity =
					BX_HD_THIS s[1].cdrom.cd->capacity();
			}
			else {
				fprintf(stderr,
...
Bit32u
	bx_hard_drive_c::read_handler(void *this_ptr, Bit32u address,
								  unsigned io_len)
{
#if !BX_USE_HD_SMF
...
// #warning "remove this hack for tomsrtbt"
// return 0;
			bx_panic
				("disk: IO read(1f0h) with drq == 0: last command was %02xh\n",
		...
		case 0x21:				// read sectors, without retries
			if (io_len != 2) {
				bx_panic("disk: %d-bytes (non-word) IO read from %04x\n",
			...
			if (BX_SELECTED_HD.hard_drive->byteswap) {	/* FALCON disk image (byte swap) */
				...
				value16 |=
					(BX_SELECTED_CONTROLLER.
				 	buffer[BX_SELECTED_CONTROLLER.buffer_index] << 8);
			}
			else {
				value16 =
					BX_SELECTED_CONTROLLER.buffer[BX_SELECTED_CONTROLLER.
				...
				value16 |=
					(BX_SELECTED_CONTROLLER.
				 	buffer[BX_SELECTED_CONTROLLER.buffer_index + 1] << 8);
			}
			...
			if (BX_SELECTED_CONTROLLER.buffer_index >= 512) {
				...
				if (BX_SELECTED_CONTROLLER.sector_count == 0) {
					...
					ret =
						BX_SELECTED_HD.hard_drive->
						lseek((loff_t) logical_sector * 512, SEEK_SET);

					if (ret == -1)
						bx_panic
							("disk: could lseek() hard drive image file\n");
					ret =
						BX_SELECTED_HD.hard_drive->
						read((bx_ptr_t) BX_SELECTED_CONTROLLER.buffer,
							 512);
					if (ret < 512) {
						bx_printf("logical sector was %u\n",
								  (unsigned) logical_sector);
						bx_panic
							("disk: could not read() hard drive image file\n");
					}
		...
		case 0xa1:
			if (bx_options.newHardDriveSupport) {
...
#else	/* FALCON (byte swap) */
					value32 = (value32 << 8) | BX_SELECTED_CONTROLLER.buffer[index];
#endif
					index++;
				}
				if (io_len == 4) {
					value32 |=
						(BX_SELECTED_CONTROLLER.buffer[index] << 16);
					value32 |=
						(BX_SELECTED_CONTROLLER.buffer[index + 1] << 24);
#if 1	/* FALCON (word swap for long access to data register) */
					value32 = ((value32 >> 16) & 0x0000ffff) | ((value32 & 0x0000ffff) << 16);
#endif
					index += 2;
				}
				...
				bx_panic("disk: IO read(1f0h): current command is %02xh\n",
						 (unsigned) BX_SELECTED_CONTROLLER.
						 current_command);
		...
		case 0xa0:{
				...
				if (index >= 2048) {
					...
					case 0xa8:	// read (12)
#ifdef LOWLEVEL_CDROM
						BX_SELECTED_HD.cdrom.cd->
							read_block(BX_SELECTED_CONTROLLER.buffer,
						...
						if (bx_dbg.disk
							|| (CDROM_SELECTED
								&& bx_dbg.cdrom)) if (!BX_SELECTED_HD.
													  cdrom.
													  remaining_blocks)
									bx_printf
									("disk: Last READ block loaded {CDROM}\n");
							else
								bx_printf
									("disk: READ block loaded (%d remaining) {CDROM}\n",
									 BX_SELECTED_HD.cdrom.
									 remaining_blocks);
				...
				if (BX_SELECTED_CONTROLLER.buffer_index >=
					(unsigned) BX_SELECTED_HD.atapi.drq_bytes) {
					BX_SELECTED_CONTROLLER.status.drq = 0;

					BX_SELECTED_HD.atapi.total_bytes_remaining -=
						BX_SELECTED_HD.atapi.drq_bytes;
					...
					if (BX_SELECTED_HD.atapi.total_bytes_remaining > 0) {
						...
						if (BX_SELECTED_HD.atapi.total_bytes_remaining <
	...
	case 0xf00009:				// hard disk sector count / interrupt reason
		...
		bx_printf
			("disk: IO read(0x1f2): current command not read/write (%x)\n",
	...
	case 0xf00019:				// hard disk drive and head register
		...
		// b3..0 HD3..HD0
		value8 = (1 << 7) |
			((BX_SELECTED_CONTROLLER.lba_mode > 0) << 6) | (1 << 5) |	// 01b = 512 sector size
...
//	case 0xf0001f:				// Hard Disk Status
		if (BX_HD_THIS drive_select && !bx_options.diskd.present) {
			// (mch) Just return zero for these registers
			value8 = 0;
		}
		else {
			value8 = (	/* mozna jsou nasledujici bity presne reverzne */
#if 1	/* IBM IDE interface */
...
#else	/* FALCON IDE INTERFACE */
					  (BX_SELECTED_CONTROLLER.status.busy) |
					  (BX_SELECTED_CONTROLLER.status.drive_ready << 1) |
					  (BX_SELECTED_CONTROLLER.status.write_fault << 2) |
					  (BX_SELECTED_CONTROLLER.status.seek_complete << 3) |
					  (BX_SELECTED_CONTROLLER.status.drq << 4) |
					  (BX_SELECTED_CONTROLLER.status.corrected_data << 5) |
					  (BX_SELECTED_CONTROLLER.status.index_pulse << 6) |
					  (BX_SELECTED_CONTROLLER.status.err << 7));
#endif
			BX_SELECTED_CONTROLLER.status.index_pulse_count++;
			BX_SELECTED_CONTROLLER.status.index_pulse = 0;
			if (BX_SELECTED_CONTROLLER.status.index_pulse_count >=
				INDEX_PULSE_CYCLE) {
				BX_SELECTED_CONTROLLER.status.index_pulse = 1;
...
/*
	case 0xf0001d:				// Hard Disk Address Register
...
*/
	default:
		bx_panic("hard drive: io read to address %x unsupported - TOS %x\n",
				 (unsigned) address, showPC());
	}
  ...
  return_value16:
	if (bx_dbg.disk || (CDROM_SELECTED && bx_dbg.cdrom))
		bx_printf("disk: 16-bit read from %04x = %04x {%s} TOS %x\n",
				  (unsigned) address, value16, DEVICE_TYPE_STRING, showPC());
	return value16;

  return_value8:
	if (bx_dbg.disk || (CDROM_SELECTED && bx_dbg.cdrom))
		bx_printf("disk: 8-bit read from %04x = %02x {%s} TOS %x\n",
				  (unsigned) address, value8, DEVICE_TYPE_STRING, showPC());
	return value8;
}
...
void
	bx_hard_drive_c::write_handler(void *this_ptr, Bit32u address,
...
void bx_hard_drive_c::write(Bit32u address, Bit32u value, unsigned io_len)
{
	...
	if (bx_dbg.disk || (CDROM_SELECTED && bx_dbg.cdrom)) {
		switch (io_len) {
		case 1:
			bx_printf("disk: 8-bit write to %04x = %02x {%s} TOS %x\n",
					  (unsigned) address, (unsigned) value,
					  DEVICE_TYPE_STRING, showPC());
			break;

		case 2:
			bx_printf("disk: 16-bit write to %04x = %04x {%s} TOS %x\n",
					  (unsigned) address, (unsigned) value,
					  DEVICE_TYPE_STRING, showPC());
			break;
/*
...
// return;
		...
		case 0x30:
			...
			if (BX_SELECTED_CONTROLLER.buffer_index >= 512) {
				...
				ret =
					BX_SELECTED_HD.hard_drive->
					write((bx_ptr_t) BX_SELECTED_CONTROLLER.buffer, 512);
				if (ret < 512)
					bx_panic
						("disk: could not write() hard drive image file\n");
		...
		case 0xa0:				// PACKET
			if (BX_SELECTED_CONTROLLER.buffer_index >= PACKET_SIZE)
				bx_panic
					("disk: IO write(1f0): buffer_index >= PACKET_SIZE\n");
			BX_SELECTED_CONTROLLER.buffer[BX_SELECTED_CONTROLLER.
										  buffer_index] = value;
			BX_SELECTED_CONTROLLER.buffer[BX_SELECTED_CONTROLLER.
										  buffer_index + 1] = (value >> 8);
			...
			if (BX_SELECTED_CONTROLLER.buffer_index >= PACKET_SIZE) {
				...
				case 0x00:		// test unit ready
					if (BX_SELECTED_HD.cdrom.ready) {
						...
						atapi_cmd_error(SENSE_NOT_READY,
				...
				case 0x03:{	// request sense
						int alloc_length =
							BX_SELECTED_CONTROLLER.buffer[4];
						init_send_atapi_command(atapi_command, 18,
												alloc_length);

						// sense data
						BX_SELECTED_CONTROLLER.buffer[0] = 0x70 | (1 << 7);
						BX_SELECTED_CONTROLLER.buffer[1] = 0;
						BX_SELECTED_CONTROLLER.buffer[2] =
							BX_SELECTED_HD.sense.sense_key;
						BX_SELECTED_CONTROLLER.buffer[3] =
							BX_SELECTED_HD.sense.information.arr[0];
						BX_SELECTED_CONTROLLER.buffer[4] =
							BX_SELECTED_HD.sense.information.arr[1];
						BX_SELECTED_CONTROLLER.buffer[5] =
							BX_SELECTED_HD.sense.information.arr[2];
						BX_SELECTED_CONTROLLER.buffer[6] =
							BX_SELECTED_HD.sense.information.arr[3];
						BX_SELECTED_CONTROLLER.buffer[7] = 17 - 7;
						BX_SELECTED_CONTROLLER.buffer[8] =
							BX_SELECTED_HD.sense.specific_inf.arr[0];
						BX_SELECTED_CONTROLLER.buffer[9] =
							BX_SELECTED_HD.sense.specific_inf.arr[1];
						BX_SELECTED_CONTROLLER.buffer[10] =
							BX_SELECTED_HD.sense.specific_inf.arr[2];
						BX_SELECTED_CONTROLLER.buffer[11] =
							BX_SELECTED_HD.sense.specific_inf.arr[3];
						BX_SELECTED_CONTROLLER.buffer[12] =
							BX_SELECTED_HD.sense.asc;
						BX_SELECTED_CONTROLLER.buffer[13] =
							BX_SELECTED_HD.sense.ascq;
						BX_SELECTED_CONTROLLER.buffer[14] =
							BX_SELECTED_HD.sense.fruc;
						BX_SELECTED_CONTROLLER.buffer[15] =
							BX_SELECTED_HD.sense.key_spec.arr[0];
						BX_SELECTED_CONTROLLER.buffer[16] =
							BX_SELECTED_HD.sense.key_spec.arr[1];
						BX_SELECTED_CONTROLLER.buffer[17] =
							BX_SELECTED_HD.sense.key_spec.arr[2];
				...
				case 0x1b:{	// start stop unit
						...
						Boolean LoEj =
							(BX_SELECTED_CONTROLLER.buffer[4] >> 1) & 1;
						Boolean Start =
							(BX_SELECTED_CONTROLLER.buffer[4] >> 0) & 1;
						...
						if (!LoEj && !Start) {	// stop the disc
							...
							bx_panic
								("disk: Eject the disc not implemented\n");
						}
						else {	// Load the disc
				...
				case 0xbd:{	// mechanism status
						uint16 alloc_length =
							read_16bit(BX_SELECTED_CONTROLLER.buffer + 8);

						if (alloc_length == 0)
							bx_panic
								("disk: Zero allocation length to MECHANISM STATUS not impl.\n");

						init_send_atapi_command(atapi_command, 8,
												alloc_length);
				...
				case 0x5a:{	// mode sense
						uint16 alloc_length =
							read_16bit(BX_SELECTED_CONTROLLER.buffer + 7);

						Bit8u PC = BX_SELECTED_CONTROLLER.buffer[2] >> 6;
						Bit8u PageCode =
							BX_SELECTED_CONTROLLER.buffer[2] & 0x3f;

						switch (PC) {
						case 0x0:	// current values
							switch (PageCode) {
							case 0x01:	// error recovery
								init_send_atapi_command(atapi_command,
														sizeof
														(error_recovery_t)
														+ 8, alloc_length);

								init_mode_sense_single(&BX_SELECTED_HD.
													   cdrom.current.
													   error_recovery,
													   sizeof
													   (error_recovery_t));
							...
							case 0x2a:	// CD-ROM capabilities & mech. status
								init_send_atapi_command(atapi_command, 28,
														alloc_length);
								init_mode_sense_single
									(&BX_SELECTED_CONTROLLER.buffer[8],
									 28);
								...
								BX_SELECTED_CONTROLLER.buffer[20] =
									(512 >> 8) & 0xff;
								BX_SELECTED_CONTROLLER.buffer[21] =
									512 & 0xff;
								BX_SELECTED_CONTROLLER.buffer[22] =
									(706 >> 8) & 0xff;
								BX_SELECTED_CONTROLLER.buffer[23] =
									706 & 0xff;
							...
							default:
								...
								bx_printf
									("cdrom: MODE SENSE PC=%x, PageCode=%x,"
				...
				case 0x12:{	// inquiry
						...
						init_send_atapi_command(atapi_command, 36,
												alloc_length);
						...
						int i;
						for (i = 0; i < 8; i++)
							BX_SELECTED_CONTROLLER.buffer[8 + i] =
								vendor_id[i];

						// Product ID
						const char *product_id = "Turbo CD-ROM\0\0\0\0";
						for (i = 0; i < 16; i++)
							BX_SELECTED_CONTROLLER.buffer[16 + i] =
								product_id[i];

						// Product Revision level
						const char *rev_level = "R0\0\0";
						for (i = 0; i < 4; i++)
							BX_SELECTED_CONTROLLER.buffer[32 + i] =
								rev_level[i];
				...
				case 0x25:{	// read cd-rom capacity
						...
						if (BX_SELECTED_HD.cdrom.ready) {
							uint32 capacity =
								BX_SELECTED_HD.cdrom.capacity;
							bx_printf
								("disk: Capacity is %d sectors (%d bytes)\n",
								 capacity, capacity * 2048);
							BX_SELECTED_CONTROLLER.buffer[0] =
								(capacity >> 24) & 0xff;
							BX_SELECTED_CONTROLLER.buffer[1] =
								(capacity >> 16) & 0xff;
							BX_SELECTED_CONTROLLER.buffer[2] =
								(capacity >> 8) & 0xff;
							BX_SELECTED_CONTROLLER.buffer[3] =
							...
							BX_SELECTED_CONTROLLER.buffer[6] =
								(2048 >> 8) & 0xff;
							BX_SELECTED_CONTROLLER.buffer[7] =
								(2048 >> 0) & 0xff;
							ready_to_send_atapi();
						}
						else {
				...
				case 0xbe:{	// read cd
						if (BX_SELECTED_HD.cdrom.ready) {
							bx_panic
								("Read CD with CD present not implemented\n");
						}
						else {
							atapi_cmd_error(SENSE_NOT_READY,
											ASC_MEDIUM_NOT_PRESENT);
				...
				case 0x43:{	// read toc
						if (BX_SELECTED_HD.cdrom.ready) {
							...
							bool msf =
								(BX_SELECTED_CONTROLLER.
								 buffer[1] >> 1) & 1;
							uint8 starting_track =
								BX_SELECTED_CONTROLLER.buffer[6];
#endif
							uint16 alloc_length =
								read_16bit(BX_SELECTED_CONTROLLER.buffer +
										   7);

							uint8 format =
								(BX_SELECTED_CONTROLLER.buffer[9] >> 6);
							int i;
							switch (format) {
							case 0:
#ifdef LOWLEVEL_CDROM
								int toc_length;
								if (!BX_SELECTED_HD.cdrom.cd->
									read_toc(BX_SELECTED_CONTROLLER.buffer,
											 &toc_length, msf,
											 starting_track)) {
									atapi_cmd_error(SENSE_ILLEGAL_REQUEST,
													ASC_INV_FIELD_IN_CMD_PACKET);
							...
							case 1:
								...
								init_send_atapi_command(atapi_command, 12,
														alloc_length);
								...
								for (i = 0; i < 8; i++)
									BX_SELECTED_CONTROLLER.buffer[4 + i] =
										0;
							...
							default:
								bx_panic
									("disk: (READ TOC) Format %d not supported\n",
									 format);
								break;
							}
						}
				...
				case 0x28:{	// read (10)
						uint32 transfer_length =
							read_16bit(BX_SELECTED_CONTROLLER.buffer + 7);
						uint32 lba =
							read_32bit(BX_SELECTED_CONTROLLER.buffer + 2);
						...
						if (transfer_length == 0) {
							...
							raise_interrupt();
							bx_printf
								("disk: READ(10) with transfer length 0, ok\n");
							break;
						}

						if (lba + transfer_length >
							BX_SELECTED_HD.cdrom.capacity) {
							atapi_cmd_error(SENSE_ILLEGAL_REQUEST,
				...
				case 0x1e:{	// prevent/allow medium removal
						if (BX_SELECTED_HD.cdrom.ready) {
							BX_SELECTED_HD.cdrom.locked =
								BX_SELECTED_CONTROLLER.buffer[4] & 1;
							atapi_cmd_nop();
						}
						else {
							atapi_cmd_error(SENSE_NOT_READY,
				...
				case 0x42:{	// read sub-channel
						...
						if (BX_SELECTED_HD.cdrom.ready) {
							...
							if (sub_q) {	// !sub_q == header only
								bx_panic
									("Read sub-channel with SubQ not implemented\n");
							}

							init_send_atapi_command(atapi_command, ret_len,
													alloc_length);
							ready_to_send_atapi();
						}
					}
...
*/
	...
	case 0xf00019:				// hard disk drive and head register
		...
		if ((value & 0xa0) != 0xa0)	// 1x1xxxxx
			bx_printf("disk: IO write 1f6 (%02x): not 1x1xxxxxb\n",
					  (unsigned) value);
		BX_HD_THIS drive_select = (value >> 4) & 0x01;
		WRITE_HEAD_NO(value & 0xf);
		if (BX_SELECTED_CONTROLLER.lba_mode == 0
			&& ((value >> 6) & 1) ==
			1) bx_printf("disk: enabling LBA mode\n");
		WRITE_LBA_MODE((value >> 6) & 1);
		break;

	case 0xf0001d:				// hard disk command
//	case 0xf0001f:				// hard disk command
		// (mch) Writes to the command register with drive_select != 0
		// are ignored if no secondary device is present
		if (BX_HD_THIS drive_select != 0 && value != 0x90
			&& !bx_options.diskd.present) break;
		...
		case 0x10:				// calibrate drive
			...
			if (BX_HD_THIS drive_select != 0 && !bx_options.diskd.present) {
				...
				raise_interrupt();
				bx_printf
					("disk: calibrate drive != 0, with diskd not present\n");
				break;
			}
		...
		case 0x21:				// read multiple sectors, without retries
			...
			ret =
				BX_SELECTED_HD.hard_drive->lseek((loff_t) logical_sector *
												 512, SEEK_SET);

			if (ret == -1) {
				bx_panic
					("disk: could not lseek() hard drive image file\n");
			}

			ret =
				BX_SELECTED_HD.hard_drive->
				read((bx_ptr_t) BX_SELECTED_CONTROLLER.buffer, 512);
			if (ret < 512) {
				bx_printf("logical sector was %u\n",
						  (unsigned) logical_sector);
		...
		case 0x91:				// initialize drive parameters
			if (BX_SELECTED_CONTROLLER.status.busy) {
				bx_panic
					("disk: init drive parameters command: BSY bit set\n");
			}
			if (BX_SELECTED_HD.device_type != IDE_DISK)
				bx_panic
					("disk: initialize drive parameters issued to non-disk\n");
			...
			if (BX_SELECTED_CONTROLLER.sector_count !=
				BX_SELECTED_HD.hard_drive->
				sectors)
bx_panic("disk: init drive params: sector count doesnt match\n");
			if (BX_SELECTED_CONTROLLER.head_no !=
				(BX_SELECTED_HD.hard_drive->heads - 1))
				bx_panic
					("disk: init drive params: head number doesn't match\n");
			BX_SELECTED_CONTROLLER.status.busy = 0;
			BX_SELECTED_CONTROLLER.status.drive_ready = 1;
			BX_SELECTED_CONTROLLER.status.drq = 0;
			BX_SELECTED_CONTROLLER.status.err = 0;
		...
		case 0xc6:				// (mch) set multiple mode
			...
			BX_SELECTED_CONTROLLER.sectors_per_block =
...
Bit32u bx_hard_drive_c::calculate_logical_address()
{
...
void bx_hard_drive_c::increment_address()
{
	BX_SELECTED_CONTROLLER.sector_count--;
	...
	if (BX_SELECTED_CONTROLLER.lba_mode) {
		...
		BX_SELECTED_CONTROLLER.sector_no++;
		if (BX_SELECTED_CONTROLLER.sector_no >
			BX_SELECTED_HD.hard_drive->sectors) {
			BX_SELECTED_CONTROLLER.sector_no = 1;
			BX_SELECTED_CONTROLLER.head_no++;
			if (BX_SELECTED_CONTROLLER.head_no >=
				...
				BX_SELECTED_CONTROLLER.head_no = 0;
				BX_SELECTED_CONTROLLER.cylinder_no++;
				if (BX_SELECTED_CONTROLLER.cylinder_no >=
					BX_SELECTED_HD.hard_drive->
					cylinders) BX_SELECTED_CONTROLLER.cylinder_no =
						BX_SELECTED_HD.hard_drive->cylinders - 1;
			}
		}
	}
}

void bx_hard_drive_c::identify_ATAPI_drive(unsigned drive)
{
	unsigned i;
	...
	BX_SELECTED_HD.id_drive[0] =
		(2 << 14) | (5 << 8) | (1 << 7) | (2 << 5) | (0 << 0);	// Removable CDROM, 50us response, 12 byte packets
...
void bx_hard_drive_c::identify_drive(unsigned drive)
{
	unsigned i;
	Bit32u temp32;
	Bit16u temp16;
	...
	BX_SELECTED_HD.id_drive[59] =
		0x0100 | BX_SELECTED_CONTROLLER.sectors_per_block;
	...
	BX_SELECTED_HD.id_drive[4] =
		(512 * BX_SELECTED_HD.hard_drive->sectors);
...
void
	bx_hard_drive_c::init_send_atapi_command(Bit8u command, int req_length,
											 int alloc_length, bool lazy)
{
	...
	BX_SELECTED_HD.atapi.total_bytes_remaining =
		(req_length < alloc_length) ? req_length : alloc_length;
...
void bx_hard_drive_c::atapi_cmd_error(sense_t sense_key, asc_t asc)
{
...
void bx_hard_drive_c::atapi_cmd_nop()
{
	BX_SELECTED_CONTROLLER.interrupt_reason.i_o = 1;
	BX_SELECTED_CONTROLLER.interrupt_reason.c_d = 1;
	BX_SELECTED_CONTROLLER.interrupt_reason.rel = 0;
...
void bx_hard_drive_c::init_mode_sense_single(const void *src, int size)
{
	// Header
...
void bx_hard_drive_c::ready_to_send_atapi()
{
	raise_interrupt();
}

void bx_hard_drive_c::raise_interrupt()
{
...
void bx_hard_drive_c::command_aborted(unsigned value)
{
...
int default_image_t::open(const char *pathname)
{
	...
	if (fd < 0) {
		/* We'll try mode O_RDONLY
		 * perhaps the user is using the write cache
		 * in "never" mode and accessing the physical drive
		 * (e.g. /dev/hda) -- TomV
		 */
...
{
	// return pluginWCRead(fd, buf, count);
	if (fd > -1) {
...
{
	// return pluginWCWrite(fd, (void *) buf, count);
	if (fd > -1) {
