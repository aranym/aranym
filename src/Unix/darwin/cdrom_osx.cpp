...
cdrom_interface::init(void) {
  D(bug("cdrom_osx.cc,v 0.1 2001/12/5 sbrumbaugh scott@openosx.com"));
  D(bug("file = '%s'",path));
}
...
cdrom_interface::insert_cdrom(char *dev)
{
  ...
  if ( strlen( CDDevicePath ) ) {
    ...
    if ( read( fd, buffer, 2048 ) == -1 ) {
      panicbug( "Unable to read from CDROM error %d", errno );
    }

  }

  D(bug( "Found CD at %s opened on descriptor %d", CDDevicePath, fd )); 
...
cdrom_interface::read_toc(uint8* buf, int* length, bool msf, int start_track)
{
  ...
  D(panicbug( "Read TOC - Not Implemented" ));
  return false;
}
