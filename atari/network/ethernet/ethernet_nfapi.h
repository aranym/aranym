...
enum {
	...
	XIF_IRQ,			/* (ethX), invokes interrupt on Atari(?) */
	XIF_START,			/* (ethX), called on 'ifup', starts receiver thread */
	XIF_STOP,			/* (ethX), called on 'ifdown', stops the thread */
	XIF_READLENGTH,		/* (ethX), return size of network data block to read */
	XIF_READBLOCK,		/* (ethX, buffer, size), read block of network data */
	XIF_WRITEBLOCK,		/* (ethX, buffer, size), writes block of network data */
